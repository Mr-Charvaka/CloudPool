...
# Find closing main tag. In index.html, there is `<div class="page" id="page-settings">` which is the last page.
# We should inject right BEFORE `</div>` that closes `<div id="main">`.
# A robust way is to find `<div class="page" id="page-settings">` and then find its closing `</div>` and the next `</div>`.
# But simpler: just inject right before the `<!-- SETTINGS MODALS -->` comment if it exists, or before `  </div>\n</div>\n\n<!-- FOOTER -->`
main_end_marker = "  </div>\n\n</div>\n\n<!-- FOOTER -->"
if main_end_marker not in content:
    main_end_marker = "  </div>\n</div>\n\n<!-- FOOTER -->"

# Let's verify if that marker exists
if main_end_marker not in content:
    # Alternative: find id="main" and count divs? Too complex.
    # We can inject BEFORE `  </div>\n</div>\n\n<!-- FOOTER -->`
    # Let's just find `<!-- FOOTER -->` and trace back to the `  </div>\n</div>`
    footer_idx = content.find("<!-- FOOTER -->")
    if footer_idx != -1:
        # Search backwards for `  </div>\n</div>`
        content = content[:footer_idx-16] + pages_injection + "\n" + content[footer_idx-16:]
...
if "if(pageId === 'kvstore')" not in content:
    ...
    # Find function goTo(pageId, el) { ... }
    match = re.search(r'function goTo\(pageId, el\)\s*\{', content)
    if match:
        idx = match.end()
        content = content[:idx] + goto_injection + content[idx:]
