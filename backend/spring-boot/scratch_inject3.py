...
# 2. Pages injection. We will inject right before `<!-- FOOTER -->`. 
# Wait, `<!-- FOOTER -->` is outside `<div id="main">`. The structure is:
#   </div> <!-- end main -->
# </div> <!-- end layout -->
# <!-- FOOTER -->
# So we need to inject BEFORE the closing `</div>` of `main`.
# Let's replace:
#   </div>
# </div>
#
# <!-- FOOTER -->
# with our pages, and then the closing tags.
# Actually, the easiest is to find `<!-- FOOTER -->` and trace back exactly.
pages_injection = """
    <!-- KV STORE -->
...
# We'll just look for the last page: <div class="page" id="page-settings">
settings_page_idx = content.find('<div class="page" id="page-settings">')
if settings_page_idx != -1:
    # Inject our new pages right before the settings page, which is safely inside the main layout
    if 'id="page-kvstore"' not in content:
        content = content[:settings_page_idx] + pages_injection + "\n" + content[settings_page_idx:]
...
if "function loadKvKeys()" not in content:
    ...
    if script_close != -1:
        content = content[:script_close] + js_injection + "\n" + content[script_close:]


# 4. Modify switchProject
...
if "if(pageId === 'kvstore')" not in content:
    match = re.search(r'function goTo\(pageId, el\)\s*\{', content)
    if match:
        idx = match.end()
        content = content[:idx] + "\n" + goto_injection + content[idx:]
