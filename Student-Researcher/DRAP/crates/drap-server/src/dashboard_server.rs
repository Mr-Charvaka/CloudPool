...
impl DashboardServer {
    ...
    pub async fn run(&self) -> anyhow::Result<()> {
        ...
        if auth_token.is_none() {
            warn!("Dashboard API has no authentication configured; set DRAP_AUTH_TOKEN to protect it");
        }
...
async fn replay_request(
    ...
    if let Some(req) = inspector.get_request_by_id(&id).await {
        if let Some(tunnel) = state.router.get_tunnel(&req.tunnel_id) {
            let raw_request = match req.raw_request {
                Some(r) => r,
                None => {
...
async fn static_handler(uri: axum::http::Uri) -> impl IntoResponse {
    ...
    if path.is_empty() || path == "index.html" {
        return serve_asset("index.html").unwrap_or_else(|status| status.into_response());
    }

    match serve_asset(path) {
        Ok(body) => body,
        Err(_) => serve_asset("index.html").unwrap_or_else(|status| status.into_response()), // SPA fallback
    }
}
