...
impl DataServer {
    pub fn new(addr: &str, router: Arc<Router>, inspector: Arc<Inspector>, tls_config: Option<rustls::ServerConfig>) -> Self {
        Self {
            addr: addr.to_string(),
            router,
            inspector,
            ip_limiters: DashMap::new(),
            tls_acceptor: tls_config.map(|cfg| TlsAcceptor::from(Arc::new(cfg))),
        }
    }
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("Data server listening on {} (TLS: {})", self.addr, 
            if self.tls_acceptor.is_some() { "enabled (experimental - use reverse proxy for production)" } else { "off - behind reverse proxy" });
    ...
    async fn handle_connection(
        ...
        let metadata = match req.parse(&buf[..n]) {
            ...
            Ok(httparse::Status::Partial) => {
                // If it's a partial parse, we cannot route it.
                return Ok(());
            }
            Err(_) => return Ok(()),
        };
        ...
        if is_websocket {
            ...
            // Forward the leftover body data from the initial buffer read
            if n > parsed_len {
                let leftover = Bytes::copy_from_slice(&buf[parsed_len..n]);
                control_tx.send(ControlMessage::Data { stream_id, data: leftover }).await?;
            }
        }
