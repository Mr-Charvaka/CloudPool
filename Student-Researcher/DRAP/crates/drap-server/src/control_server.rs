...
impl ControlServer {
    pub fn new(tls_config: rustls::ServerConfig, addr: &str, router: Arc<Router>, auth_token: String) -> Self {
        Self {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = Self::handle_connection(acceptor, stream, router, auth_token).await {
