...
impl Database {
    ...
    pub async fn get_history(&self, limit: i64) -> Result<Vec<CapturedRequest>> {
        let rows = sqlx::query(
            r#"
            SELECT id, tunnel_id, timestamp, method, path, host, headers, duration_ms, hex_snippet
        ...
        for row in rows {
            ...
            let hex_snippet: Option<String> = row.try_get("hex_snippet").ok();
